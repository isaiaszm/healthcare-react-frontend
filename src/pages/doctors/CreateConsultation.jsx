...
export const CreateConsultation = () => {
  ...
  const fetchAppointmentDetails = async () => {
    try {
      const response = await apiService.getMyAppointments(appointmentId);

      if (response.data.statusCode === 200) {
        const foudnAppointment = response.data.data.find(
          (appt) => appt.id === parseInt(appointmentId),
        );
        if (foudnAppointment) {
          setAppointment(foudnAppointment);
      ...
      <div className="form-container">
        ...
        {appointment && (
          <div className="consultation-header-info">
            ...
            <div className="appointment-summary">
              <p>
                <strong>Patient:</strong> {appointment.patient.firstName}{" "}
                {appointment.patient.lastName}
              </p>
              <p>
                <strong>Date:</strong> {formatDateTime(appointment.startTime)}
              </p>
              <p>
                <strong>Purpose:</strong> {appointment.purposeOfConsultation}
              </p>
              <p>
                <strong>Symptoms:</strong> {appointment.initialSymptoms}
              </p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            />
            <small className="form-help">
              Objective observations and examination findings
            </small>
          </div>
